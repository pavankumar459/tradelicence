...
public class LicenseStatusServiceTest {
	...
	public void testAcreateLicenseStatus() {
		...
		licenseStatus.setActive(active);
		...
		try {
			LicenseStatusResponse licenseStatusResponse = licenseStatusService.createLicenseStatusMaster( licenseStatusRequest );
	...
	public void testAsearchLicenseStatus() {
		...
		try {
			LicenseStatusResponse licenseStatusResponse = licenseStatusService.getLicenseStatusMaster(requestInfo, tenantId, new Integer[] { LicenseId.intValue() },
	...
	public void testBmodifyLicenseName() {
		...
		try {
			LicenseStatusResponse licenseStatusResponse = licenseStatusService.updateLicenseStatusMaster(licenseStatusRequest);
	...
	public void testBsearchUpdatedLicenseName() {
		...
		try {
			LicenseStatusResponse licenseStatusResponse = licenseStatusService.getLicenseStatusMaster(requestInfo, tenantId, new Integer[] { LicenseId.intValue() },
					NameToupdate, code, active, pageSize, offset);
			if (licenseStatusResponse.getLicenseStatuses().size() == 0)
				assertTrue(false);
	...
	public void testCsearchUpdatedLicenseCode() {
		...
		try {
			LicenseStatusResponse licenseStatusResponse = licenseStatusService.getLicenseStatusMaster(requestInfo, tenantId, new Integer[] { LicenseId.intValue() },
					NameToupdate, CodeToUpdate, active, pageSize, offset);
			if (licenseStatusResponse.getLicenseStatuses().size() == 0)
				assertTrue(false);
